...
public class Workout_Model {
    ...
    public static List<List<String>> getWorkouts(){
        ...
        try (BufferedReader woBr = new BufferedReader(new FileReader("DreamTeam.v1/workouts.csv"))) {
            String line;
    ...
    public static void addWorkouts(String username, String workoutName, String comment, String totalDuration, Date date, String[] exercises, String[] durations) {
        List<List<String>> workouts = getWorkouts();
        File workoutFile = new File("DreamTeam.v1/workouts.csv");
        try{
            ...
            for(int i=0;i<exercises.length;i++){
                workoutWriter.write(","+exercises[i] + "," + durations[i]);
            }
    ...
    public void editWorkout(String newUsername, String newWOName, String newComment, String newTotal, Date date, String[] newExercises, String[] newDurations) throws ParseException {
        List<List<String>>workouts = getWorkouts();
        for(int i=0; i<workouts.size(); i++){
        ...
        for(int i =0; i< workouts.size();i++){
            ...
            for(int j=5;j<(workouts.get(i).size()-5);j++){
                ...
                durations[j-4] = workouts.get(i).get(j+1);
            }
        }
        File workoutFile = new File("DreamTeam.v1/workouts.csv");
        try {
